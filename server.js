...
app.get('/api/orders', requireAdmin, async (req, res) => {
  try {
    ...
    const result = await dbQuery(
      `SELECT *, TO_CHAR(created_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"') AS created_at FROM orders ${where} ORDER BY created_at DESC LIMIT $${params.length-1} OFFSET $${params.length}`,
      params
    );
