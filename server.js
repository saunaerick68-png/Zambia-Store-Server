...
async function initDB() {
  try {
    ...
    // Add category_slug column if upgrading from old schema
    await dbQuery(`
...
// ═══════════════════════════════════════════════════════════════════════════════
// Serve static files (index.html lives next to server.js)
// ═══════════════════════════════════════════════════════════════════════════════
    ...
    try {
      ...
      const result = await dbQuery(`
        UPDATE products SET
          ...
          store_image=$14, carousel_images=$15, detail_images=$16, updated_at=NOW()
...
app.get('/api/products/:id/reviews', async (req, res) => {
  try {
    ...
    const result = await dbQuery(
      `SELECT r.*, TO_CHAR(r.created_at, 'DD Mon YYYY') AS date_label
...
app.patch('/api/reviews/:id/approve', requireAdmin, async (req, res) => {
  try {
    const result = await dbQuery(
      'UPDATE reviews SET is_approved = NOT is_approved WHERE id=$1 RETURNING id, is_approved',
      [req.params.id]
    );
    if (!result.rows.length) return sendError(res, 404, 'Review not found.');
    return res.json({ success: true, is_approved: result.rows[0].is_approved });
  } catch (err) {
    return sendError(res, 500, 'Failed to update review.', err.message);
  }
});
...
app.get('/api/notifications', requireAdmin, async (req, res) => {
  try {
    const result = await dbQuery(`
      SELECT n.*, o.order_number, o.product_name, o.total, o.payment_method, o.city
